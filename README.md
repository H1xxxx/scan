...
A simple tkinter GUI is included for Windows. Place `hscan_gui.py` next to `hscan.exe` and run:

```bash
python hscan_gui.py
```

![HScan GUI](assets/screenshot_gui.png)

The GUI lets you pick the config file, target list, mode, timeout, and batch size, and streams HScan output into a built-in console.
...
├── hscan_gui.py         # tkinter GUI wrapper
